...
describe('RegisterComponent', () => {
  ...
  it('should make the firstName and lastName controls required and validate value', () => {
    ...
    firstNameControl?.setValue('2'); // Invalid because less than 3
    expect(firstNameControl?.valid).toBeFalsy();

    firstNameControl?.setValue('3'); // Valid because equal to 3
    expect(firstNameControl?.valid).toBeTruthy();

    firstNameControl?.setValue('21'); // Invalid because more than 20
    expect(firstNameControl?.valid).toBeFalsy();

    lastNameControl?.setValue('');
    expect(lastNameControl?.valid).toBeFalsy();

    lastNameControl?.setValue('2'); // Invalid because less than 3
    expect(lastNameControl?.valid).toBeFalsy();

    lastNameControl?.setValue('3'); // Valid because equal to 3
    expect(lastNameControl?.valid).toBeTruthy();

    lastNameControl?.setValue('21'); // Invalid because more than 20
    expect(lastNameControl?.valid).toBeFalsy();
  });
  ...
  it('should make the password control required and validate length', () => {
    ...
    passwordControl?.setValue('2'); // Invalid because less than 3
    expect(passwordControl?.valid).toBeFalsy();

    passwordControl?.setValue('3'); // Valid because equal to 3
    expect(passwordControl?.valid).toBeTruthy();

    passwordControl?.setValue('41'); // Invalid because more than 40
    expect(passwordControl?.valid).toBeFalsy();
  });

  it('should call register method on submit when form is valid', () => {
    authService.register.mockReturnValue(of(void 0)); // Mock the register method with a return value
  ...
  it('should navigate to login on successful registration', () => {
    const routerSpy = jest.spyOn(router, 'navigate'); // Spy on router.navigate
